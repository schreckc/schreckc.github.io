...
function main() {
  ...
  function drawScene() {
    ...
    gl.uniform2fv(transLocation, trans2);
    gl.drawArrays(primitiveType, offset, count);

    if (trans2[1] >= 0.5 * gl.canvas.height / gl.canvas.width || trans2[1] <= -0.5 * gl.canvas.height / gl.canvas.width) {
      step2 = -step2;
    }
    ++t;
    if (t < 1000) {
