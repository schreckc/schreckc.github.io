...
\noindent\textbf{Instructions:} The project is to be sent by mail (camille.schreck@inria.fr). The title of the mail should start with [PROJET ENSG].
The project should be a file called \emph{projet\_name\_surname.zip} with an archived repository containing your files, or a file \emph{projet\_name\_surmame.html} if you have only one.\\
\textbf{\textit{The code should be commented}}
...
\begin{enumerate}
\item In a new project, modify the \texttt{uv} coordinates and the fragment shader such that one of the triangles of the pentagon only keep its two inside borders (see Figure \ref{pentacman}).
\end{enumerate}
